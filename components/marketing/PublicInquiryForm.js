...
const initial = { full_name:'',email:'',company_name:'',message:'',applicant_type:'',country_region:'',intended_data_category:'',consent:false,data_restrictions_acknowledged:false,website:'',request_id:'',referral_code:'' }
export default function PublicInquiryForm({ earlyAccess = false }) {
  ...
  const [form,setForm] = useState(initial), [errors,setErrors] = useState({}), [pending,setPending] = useState(false), [success,setSuccess] = useState(null), [ready,setReady] = useState(false), [restored,setRestored] = useState(false), [storageAvailable,setStorageAvailable] = useState(false), [referral,setReferral] = useState(null), [referralError,setReferralError] = useState('')
  ...
  useEffect(() => {
    if (earlyAccess || !router.isReady) return
    const code = typeof router.query.ref === 'string' ? router.query.ref.trim() : ''
    if (!code) return
    setForm(current => ({ ...current, referral_code: code }))
    dispatch(apiCallBegan({ url: `/sales-partners/referrals/${encodeURIComponent(code)}`, requestKey: 'public-sales-partner-referral' }))
      .then(result => {
        if (result?.ok) { setReferral(result.payload?.data?.referral || null); setReferralError('') }
        else { setReferral(null); setReferralError('This Sales Partner link is no longer available. Ask the person who referred you for a current link.') }
      })
  },[dispatch,earlyAccess,router.isReady,router.query.ref])
  useEffect(() => {
  ...
  const clearDraft = () => { setForm({ ...initial,request_id:crypto.randomUUID(),referral_code:form.referral_code });setErrors({});setRestored(false);try {sessionStorage.removeItem(key)} catch {} }
  const submit = async event => {
    ...
    const data = Object.fromEntries((earlyAccess
      ? ['full_name','email','company_name','message','applicant_type','country_region','intended_data_category','consent','data_restrictions_acknowledged','website','request_id']
      : ['full_name','email','company_name','message','consent','website','referral_code'])
  ...
  return <form id='application' className='mk-application' onSubmit={submit} noValidate aria-labelledby={`${prefix}-title`} aria-busy={pending}>
    ...
    {referral && <p className='mk-notice' role='status'>Referred by <strong>{referral.member_name}</strong> with {referral.partner_name}. Your request will be connected to their referral record.</p>}
    {referralError && <p className='mk-form-error' role='alert'>{referralError}</p>}
    ...
    <div className='mk-form-actions'><button className='mk-button' type='submit' disabled={!ready || pending || Boolean(referralError)}>{pending ? 'Sending…' : earlyAccess ? 'Submit application' : 'Request my demo'}<ArrowRight size={17} aria-hidden /></button><button className='mk-text-button' type='button' onClick={clearDraft}>Clear answers</button></div></fieldset>
