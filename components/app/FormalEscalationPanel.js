...
export const FormalDetail = ({ item, detail, context, files, pending, record, onAction, onSource, onNewCase, onRelated, onProduction, onDraftChange, onDownload, canCreate }) => {
  ...
  const primaryProduction = affectedProductionRecords.find(production => production.primary) || (idOf(item.production_record) === idOf(record) ? record : null)
  ...
  return <div className='formalV2'>
    ...
    {item.category === 'non_conformance' && !scope && <section className='formalV2__scope'><h3>Affected parts awaiting supplier confirmation</h3><p>The affected quantity, lot, and serial details are not yet established. The supplier owns the next task: confirm affected parts.</p></section>}
    {scope && <section className='formalV2__scope'><h3>{scope.affected_quantity} affected{(scope.produced_quantity ?? primaryProduction?.quantity) != null && ` · ${Math.max(0, Number(scope.produced_quantity ?? primaryProduction.quantity) - scope.affected_quantity)} outside the reported scope`}</h3><DataSummary data={scope} {...summaryProps} />{canCreate && <Button variant='secondary' onClick={() => onNewCase('formal', item.id)}>Create a related formal record</Button>}</section>}
