import { ArrowUpRight, Check, CircleAlert, Clock3, FileUp, LoaderCircle, MessageSquareText, Paperclip, Send, ShieldAlert, Trash2 } from 'lucide-react'
...
const PartCaseDrawer = ({
  ...
  const [promotion, setPromotion] = useState({ production_record_id: '', category: 'issue', explanation: '' })
  ...
  useEffect(() => {
    ...
    setForm(current => ({ ...emptyForm, share_id: shares.length === 1 ? String(shares[0].id || shares[0]._id) : current.share_id }))
  }, [mode, open, shares.length])
  useEffect(() => {
    if (!item) return
    setMessage('')
    setActionNote('')
  ...
  return <ResponsiveDrawer open={open} title={mode === 'create' ? 'Create collaboration case' : item?.title || 'Collaboration case'} onClose={onClose} wide>
    {mode === 'create' ? <form className='partCaseForm' onSubmit={submitCreate}>
      ...
      <div className='partCaseForm__context'>
        ...
        <div><p className='technicalLabel'>Revision context</p><strong>{selectedAnchor ? selectedAnchor.label || formatLabel(selectedAnchor.anchor_kind || selectedAnchor.kind) : 'Revision-level case'}</strong><span>{sourceAsset?.attachment?.display_filename || sourceAsset?.attachment?.original_filename || 'No file reference selected'}</span></div>
      ...
      <div className='productionFormGrid'>
        ...
        <label className='selectField'><span>{relationshipLabel}</span><select required value={form.share_id} onChange={event => set('share_id', event.target.value)}><option value=''>{relationshipChoice}</option>{shares.filter(share => share.state === 'active').map(share => <option key={share.id || share._id} value={share.id || share._id}>{organizationType === 'supplier' ? (share.oem_organization?.name || 'OEM customer') : (share.supplier_organization?.name || 'Supplier')}</option>)}</select></label>
      ...
      {!!productionRecords.length && <fieldset className='partCaseLinks'><legend>Related production records (optional)</legend>{productionRecords.map(record => <label key={record.id || record._id}><input type='checkbox' checked={form.production_record_ids.includes(String(record.id || record._id))} onChange={event => set('production_record_ids', event.target.checked ? [...form.production_record_ids, String(record.id || record._id)] : form.production_record_ids.filter(id => id !== String(record.id || record._id)))} /> <span>{record.public_reference || record.po_number}</span></label>)}</fieldset>}
      ...
      <FormMessage type={feedback?.type}>{feedback?.message}</FormMessage>
      {item && <>
        <div className={`partCaseNextAction${isMyAction ? ' is-mine' : ''}`}><CircleAlert aria-hidden='true' /><div><p className='technicalLabel'>Current responsibility</p><strong>{isMyAction ? 'Your company owns the next step' : item.current_actor_side === 'none' ? 'This workflow has no remaining action' : `Waiting on ${ownerLabel}`}</strong><span>{isMyAction ? 'Review the conversation, then use a workflow decision below or send a message if you need clarification.' : item.current_actor_side === 'none' ? 'The conversation and decision record remain available for reference.' : 'You can still add context to the conversation while the other company prepares its response.'}</span></div></div>
        ...
        <form className='partCaseReply' onSubmit={event => { event.preventDefault(); if (!message.trim()) return; onMessage(message).then(result => { if (result?.ok) setMessage('') }) }}><label htmlFor='part-case-message'>Add to the conversation</label><small>Messages add context without changing the workflow owner or state.</small><textarea id='part-case-message' value={message} onChange={event => setMessage(event.target.value)} maxLength={6000} /><Button type='submit' disabled={pending || !message.trim()}><Send aria-hidden='true' /> Send message</Button></form>
        <section className='partCaseAttachments'><header><h3><Paperclip aria-hidden='true' /> Evidence and files</h3><label className={`button button--secondary${itarControlled && !itarUploadAuthorized ? ' is-disabled' : ''}`} aria-disabled={itarControlled && !itarUploadAuthorized}><FileUp aria-hidden='true' /> Attach file<input type='file' hidden disabled={itarControlled && !itarUploadAuthorized} onChange={event => { const file = event.target.files?.[0]; if (file) onUpload(file, { itar_upload_authorized: itarUploadAuthorized, synthetic_data_acknowledged: itarUploadAuthorized }); event.target.value = '' }} /></label></header>{itarControlled && <label className='productionCheck partCaseAttachments__itar'><input type='checkbox' checked={itarUploadAuthorized} onChange={event => setItarUploadAuthorized(event.target.checked)} /><ShieldAlert aria-hidden='true' /><span><strong>I am authorized to attach this ITAR-controlled data</strong><small>The evidence inherits the same protected handling and access rules as this revision.</small></span></label>}{upload && <p><LoaderCircle className='spin' aria-hidden='true' /> {upload.filename} · {upload.progress}%</p>}{itemDetail.attachments?.length ? <ul>{itemDetail.attachments.map(file => <li key={file.id || file._id}><span>{file.display_filename || file.original_filename}</span><div>{file.export_control === 'itar' && <ShieldAlert aria-hidden='true' />}<Button type='button' variant='secondary' onClick={() => onDownloadAttachment?.(file)}><Paperclip aria-hidden='true' /> Download</Button></div></li>)}</ul> : <p>No files attached.</p>}</section>
        {!!item.available_actions?.length && <section className='partCaseActions'><h3>Complete the next workflow step</h3><p>A workflow decision changes responsibility or state and is recorded in the technical audit trail.</p><label className='textAreaField' htmlFor='part-case-action-note'><span>Decision note</span><textarea id='part-case-action-note' value={actionNote} onChange={event => setActionNote(event.target.value)} placeholder='Explain the decision or next expected action' /></label><div>{item.available_actions.map(action => <Button key={action.key} type='button' variant={action.key === 'reject' ? 'danger' : action.key === 'acknowledge' ? 'secondary' : 'primary'} disabled={pending || (action.note_required && actionNote.trim().length < 2)} onClick={() => onAction(action.key, actionNote).then(result => { if (result?.ok) setActionNote('') })}>{action.key === 'acknowledge' ? <Check aria-hidden='true' /> : <Clock3 aria-hidden='true' />}{action.label}</Button>)}</div></section>}
        {!!productionRecords.length && !item.escalated_attention && <details className='partCasePromote'><summary>Promote to production attention</summary><div className='productionFormGrid'><label className='selectField'><span>Production record</span><select value={promotion.production_record_id} onChange={event => setPromotion(current => ({ ...current, production_record_id: event.target.value }))}><option value=''>Choose record</option>{productionRecords.map(record => <option key={record.id || record._id} value={record.id || record._id}>{record.public_reference || record.po_number}</option>)}</select></label><label className='selectField'><span>Attention type</span><select value={promotion.category} onChange={event => setPromotion(current => ({ ...current, category: event.target.value }))}><option value='information'>Information</option><option value='issue'>Issue</option><option value='production_block'>Production block</option><option value='non_conformance'>Non-conformance</option></select></label></div><label className='textAreaField' htmlFor='part-case-promotion'><span>Why production attention is required</span><textarea id='part-case-promotion' value={promotion.explanation} onChange={event => setPromotion(current => ({ ...current, explanation: event.target.value }))} /></label><Button type='button' disabled={pending || !promotion.production_record_id || promotion.explanation.trim().length < 8} onClick={() => onPromote(promotion)}>Promote case</Button></details>}
