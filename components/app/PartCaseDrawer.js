    ...
    {mode === 'create' ? <form className='partCaseForm' onSubmit={submitCreate}>
      ...
      {item && <>
        ...
        {item.visual_anchor && <section className='partCaseVisual'>
          <header><div><p className='technicalLabel'>Linked visual</p><h3>{item.visual_anchor.label || formatLabel(item.visual_anchor.anchor_kind || item.visual_anchor.kind)}</h3></div><Button type='button' variant='secondary' onClick={() => onOpenAnchor?.(item.visual_anchor)}><ArrowUpRight aria-hidden='true' /> Open in workspace</Button></header>
        ...
        <form className='partCaseResponse' onSubmit={submitReply}>
          ...
          <div className='partCaseResponse__modes' role='tablist' aria-label='Response type'>
            <button type='button' role='tab' aria-selected={replyMode === 'message'} className={replyMode === 'message' ? 'is-active' : ''} onClick={() => setReplyMode('message')}>Conversation</button>
            {!!item.available_actions?.length && <button type='button' role='tab' aria-selected={replyMode === 'workflow'} className={replyMode === 'workflow' ? 'is-active' : ''} onClick={() => setReplyMode('workflow')}>Workflow decision</button>}
            {canPromote && <button type='button' role='tab' aria-selected={replyMode === 'attention'} className={replyMode === 'attention' ? 'is-active' : ''} onClick={() => setReplyMode('attention')}>Production attention</button>}
          </div>
          ...
          <footer><Button type='submit' disabled={pending || !canSubmitReply}>{pending ? <LoaderCircle className='spin' aria-hidden='true' /> : <Send aria-hidden='true' />}{replyMode === 'workflow' ? selectedWorkflow?.label || 'Record decision' : replyMode === 'attention' ? 'Create attention flag' : 'Send reply'}</Button></footer>
        </form>
