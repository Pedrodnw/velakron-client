import { Archive, Box, Download, FileText, Image, LoaderCircle, MessageSquarePlus, Pencil, ShieldAlert, Trash2, Upload } from 'lucide-react'
...
const FileUploader = ({ kind, organizationType, record, itarCapability, pending, upload, feedback, onUpload }) => {
  ...
  const ready = itarControlled
    ? (itarPreview ? syntheticDataAcknowledged : !itarUnavailable && itarUploadAuthorized)
    : regulatedDataAcknowledged
  const choices = organizationType === 'oem'
    ? ['shared', 'oem_internal']
    : organizationType === 'velakron' ? ['velakron_internal'] : ['shared']
  ...
  return <form className='fileUploader' onSubmit={async event => { event.preventDefault(); const form = event.currentTarget; if (file && ready && await onUpload({ file, category, visibility, regulated_data_acknowledged: !itarControlled, synthetic_data_acknowledged: itarPreview, itar_upload_authorized: itarControlled && !itarPreview })) { setFile(null); setRegulatedDataAcknowledged(false); setItarUploadAuthorized(false); setSyntheticDataAcknowledged(false); form.reset() } }}>
    ...
    {feedback && <FormMessage type={feedback.type}>{feedback.message}</FormMessage>}
    {!itarControlled && <label className='productionCheck complianceAcknowledgement'><input type='checkbox' checked={regulatedDataAcknowledged} onChange={event => setRegulatedDataAcknowledged(event.target.checked)} /><span><strong>I confirm this file is permitted</strong><small>It does not contain ITAR, EAR-controlled, CUI, classified, or other regulated technical data.</small></span></label>}
    {itarControlled && !itarPreview && !itarUnavailable && <label className='productionCheck complianceAcknowledgement itarUploadAcknowledgement'><input type='checkbox' checked={itarUploadAuthorized} onChange={event => setItarUploadAuthorized(event.target.checked)} /><ShieldAlert aria-hidden='true' /><span><strong>I am authorized to upload this ITAR-controlled technical data</strong><small>The file inherits this record’s ITAR protection and access controls.</small></span></label>}
    {itarPreview && <label className='productionCheck complianceAcknowledgement itarUploadAcknowledgement'><input type='checkbox' checked={syntheticDataAcknowledged} onChange={event => setSyntheticDataAcknowledged(event.target.checked)} /><ShieldAlert aria-hidden='true' /><span><strong>This is synthetic preview data</strong><small>Never place real ITAR data in the local preview environment.</small></span></label>}
    {itarUnavailable && <FormMessage type='error'>ITAR uploads are locked until Velakron is running in the approved GovCloud, FIPS, database, and malware-scanning environment.</FormMessage>}
    <p className='complianceHint'>{itarPreview
      ? 'This local workflow is for synthetic demonstrations only; it does not protect real ITAR technical data.'
      : itarControlled
        ? 'The file remains unavailable until its encryption and malware checks are complete.'
        : 'Prototype files are checked for their real file format before download. Malware scanning is not enabled in this prototype.'}</p>
  </form>
}

const FileList = ({ files, itarControlled, canArchive, userId, pending, onDownload, onArchive, onView }) => {
  ...
  return <div className='productionFiles'>{files.map(file => {
    ...
    <div><strong>{file.display_filename || file.original_filename}</strong><small>{formatLabel(file.category)} · {visibilityLabel(file.visibility)} · {(file.byte_size / 1024).toFixed(1)} KB</small>{itarControlled && <span className='itarFileMarker'><ShieldAlert aria-hidden='true' /> ITAR controlled</span>}</div>
...
const ProductionCollaborationPanel = ({
  ...
  const itarControlled = record.export_control === 'itar'
  const openModel = async (file, attestation = {}) => {
    const result = await onView(file, attestation)
    if (!result?.ok) return result
    setViewingModel({ file, source: result.payload.data.view.target })
    return result
  }
  const beginFileAction = async (file, purpose) => {
    setAccessFeedback(null)
    if (itarControlled) {
      setItarRequest({ file, purpose })
      return
    }
    if (purpose === 'view') await openModel(file)
    else await onDownload(file, {})
  }
  const confirmItarAccess = async attestation => {
    setAccessPending(true)
    setAccessFeedback(null)
    const result = itarRequest?.purpose === 'view'
      ? await openModel(itarRequest.file, attestation)
      : await onDownload(itarRequest.file, attestation)
    setAccessPending(false)
    if (!result?.ok) {
      setAccessFeedback({ type: 'error', message: result?.error?.message || 'Velakron could not authorize this ITAR file access.' })
      return
    }
    setItarRequest(null)
  }
  ...
  <ResponsiveDrawer open={Boolean(viewingModel)} title={viewingModel?.file?.display_filename || viewingModel?.file?.original_filename || '3D model'} onClose={() => setViewingModel(null)} wide>
    {viewingModel && <ModelViewer file={viewingModel.file} source={viewingModel.source} />}
  </ResponsiveDrawer>
  <ItarAccessDialog file={itarRequest?.file} purpose={itarRequest?.purpose} open={Boolean(itarRequest)} pending={accessPending} feedback={accessFeedback} onClose={() => { if (!accessPending) setItarRequest(null) }} onConfirm={confirmItarAccess} />
  </>
}
