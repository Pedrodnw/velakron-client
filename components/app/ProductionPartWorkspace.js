...
import { TechnicalAcceptance } from './FormalEscalationPanel'
import { loadFormalRecords, loadProductionCollaboration, productionCollaborationSelectors } from '../../store/slices/entities/productionCollaboration'
import { workflowError } from '../../store/collaborationV2'
import { buildModelCaseMarkers, casesForProduction, modelCaseMarkersForAsset, productionVisualAnchors } from './partCaseMarkers'
...
const ProductionPartWorkspace = ({ record, organization, onEditDetails, onModelThumbnailReady }) => {
  ...
  const canReviewPart = useSelector(getHasPermission('part.review'))
  const canAcknowledgeRequirement = useSelector(getHasPermission('part.requirement_acknowledge'))
  ...
  const [revealViewer, setRevealViewer] = useState(false)
  ...
  useEffect(() => {
    if (router.query.new_conversation === '1' && v2Enabled && canCreateCase) setCaseDrawer({ open: true, mode: 'create', id: '' })
  }, [router.query.new_conversation, v2Enabled, canCreateCase])
  ...
  const productionCases = useMemo(() => casesForProduction(revisionCases, record.id), [record.id, revisionCases])
  const visibleCases = caseScope === 'production' ? productionCases : revisionCases
  const visualAnchors = productionVisualAnchors(anchors, visibleCases, requirements, formalState.formalRecords || [], selectedAnchorId)
  const modelCaseMarkers = useMemo(() => buildModelCaseMarkers(anchors, visibleCases), [anchors, visibleCases])
  const viewerCaseMarkers = useMemo(() => modelCaseMarkersForAsset(modelCaseMarkers, viewer.asset), [modelCaseMarkers, viewer.asset])
  useEffect(() => {
    setCaseScope('production')
    setSelectedAnchorId('')
  }, [record.id])
  ...
  const openCases = visibleCases.filter(item => !['closed', 'escalated'].includes(item.state))
  ...
  useEffect(() => {
    if (!revealViewer || caseDrawer.open || !viewerStageRef.current) return undefined
    // Wait for the drawer to restore page scrolling before revealing the viewer.
    const frame = requestAnimationFrame(() => {
      viewerStageRef.current?.focus({ preventScroll: true })
      viewerStageRef.current?.scrollIntoView({ block: 'start' })
      setRevealViewer(false)
    })
  ...
  const runCase = async (operation, success) => {
    ...
    const result = await operation()
    if (!result?.ok) {
      setCaseFeedback({ type: 'error', message: workflowError(result) })
      if (['VERSION_CONFLICT', 'CONVERSATION_ESCALATED', 'CONVERSATION_CLOSED'].includes(result?.error?.code) && caseDrawer.id) {
        await dispatch(loadPartCollaborationItem(caseDrawer.id))
        await refresh()
      ...
      await refresh()
      if (caseDrawer.id) await dispatch(loadPartCollaborationItem(caseDrawer.id))
      dispatch(loadProductionCollaboration(record.id))
      dispatch(loadFormalRecords(record.id))
    }
    return result
  ...
  const createCase = async form => {
    ...
    const result = await dispatch(createPartCollaboration({
      ...form,
      ...(router.query.references_formal_record ? { references_formal_record_id: router.query.references_formal_record } : {}),
      share_id: currentShareId || form.share_id,
  ...
  const closeCase = async () => {
    ...
    delete query.new_conversation
    delete query.references_formal_record
    await router.replace({ pathname: router.pathname, query }, undefined, { shallow: true })
    setCaseDrawer({ open: false, mode: 'create', id: '' })
    setPendingAnchor(null)
    setAnnotationMode(false)
  }
  const openCaseAnchor = async anchor => {
    if (!assets.some(asset => idOf(asset) === idOf(anchor?.source_asset))) {
      setCaseFeedback({ type: 'error', message: 'The file linked to this conversation is no longer available on this revision.' })
      return
    }
    // Clear the conversation URL before closing so its deep link cannot reopen it.
    await closeCase()
    focusAnchor(anchor)
    setRevealViewer(true)
  }
  ...
  return <>
    <section className='partWorkspaceShell productionPartWorkspace'>
      ...
      {tab === 'overview' && <section className='partOverview productionPartWorkspace__overview'>
        {(formalState.formalRecords || []).filter(item => item.technical_acceptance).map(item => <TechnicalAcceptance key={item.id} acceptance={item.technical_acceptance} />)}
      ...
      {['model', 'drawing'].includes(tab) && <div className='partVisualWorkspace'>
        ...
        <main className='partViewerStage' ref={viewerStageRef} tabIndex={-1} aria-label={tab === 'drawing' ? 'Full drawing viewer' : 'Full model viewer'}><div className='partViewerStage__toolbar'><div><p className='technicalLabel'>Production technical review</p><strong>{viewer.asset ? attachmentName(viewer.asset) : `Open a ${tab}`}</strong></div>{viewer.asset && <div>{canCreateCase && <Button variant={annotationMode ? 'primary' : 'secondary'} onClick={() => setAnnotationMode(value => !value)}><CircleDot aria-hidden='true' /> {annotationMode ? 'Cancel selection' : tab === 'drawing' ? 'Mark drawing area' : 'Select model feature'}</Button>}<Button variant='secondary' onClick={() => downloadAsset(viewer.asset)}><Download aria-hidden='true' /> Download</Button></div>}</div>{canCreateCase && annotationMode && <div className='partViewerSelectionGuide' role='status'><CircleDot aria-hidden='true' /><div><strong>{tab === 'drawing' ? 'Mark the exact drawing context' : 'Select the exact model feature'}</strong><span>{tab === 'drawing' ? 'Click a point or drag a rectangle around the dimension, note, or feature. Velakron will attach the sheet and view to the new case.' : 'Rotate to the relevant area, then click the visible surface. Velakron will preserve the camera and feature reference.'}</span></div><button type='button' onClick={() => setAnnotationMode(false)}>Cancel</button></div>}<PartAssetViewer asset={viewer.asset} source={viewer.source} loading={viewer.loading} annotationMode={canCreateCase && annotationMode} anchors={visualAnchors.filter(anchor => !anchor.source_asset || idOf(anchor.source_asset) === idOf(viewer.asset))} caseMarkers={viewerCaseMarkers} selectedAnchorId={selectedAnchorId} onSelect={chooseVisualAnchor} onOpenCase={openCase} onThumbnailReady={captureModelThumbnail} /></main>
        <aside className='partContextRail'><section><p className='technicalLabel'>Production snapshot</p><dl><div><dt>Revision</dt><dd>{revision.revision}</dd></div><div><dt>Quantity</dt><dd>{record.quantity} {record.unit === 'other' ? record.unit_other : formatLabel(record.unit)}</dd></div><div><dt>Required arrival</dt><dd>{formatDate(record.required_delivery_date)}</dd></div><div><dt>{organization?.type === 'supplier' ? 'OEM customer' : 'Supplier'}</dt><dd>{relatedCompanyName || 'Not assigned'}</dd></div></dl></section><section><p className='technicalLabel'>Visual references</p>{visualAnchors.length ? <ul>{visualAnchors.map(anchor => <li key={idOf(anchor)}><button type='button' onClick={() => focusAnchor(anchor)} aria-current={selectedAnchorId === idOf(anchor) ? 'true' : undefined}>{anchor.label || formatLabel(anchor.kind)}</button></li>)}</ul> : <p>No anchored cases yet.</p>}</section></aside>
      </div>}

      {tab === 'requirements' && <section className='partWorkspacePanel'><header><div><p className='technicalLabel'>Released requirements</p><h2>Revision {revision.revision} requirements</h2><p>These requirements belong to the immutable revision used by this production record.</p></div></header>{requestedRequirements.length > 0 && <div className='partRequirementProgress'><div><p className='technicalLabel'>{organization?.type === 'supplier' ? 'Your acknowledgement progress' : `${partnerName} acknowledgement progress`}</p><strong>{acknowledgedIds.size} of {requestedRequirements.length} requested requirements acknowledged</strong></div><progress max={requestedRequirements.length} value={Math.min(acknowledgedIds.size, requestedRequirements.length)} aria-label={`${acknowledgedIds.size} of ${requestedRequirements.length} requested requirements acknowledged`} /></div>}{requirements.length ? <div className='partRequirementList'>{requirements.map((item, index) => { const acknowledged = acknowledgedIds.has(idOf(item)); return <article key={idOf(item)}><span className='partRequirementList__index'>{String(index + 1).padStart(2, '0')}</span><div><div className='partRequirementList__title'><StatusBadge tone='info'>{formatLabel(item.type)}</StatusBadge><h3>{item.title}</h3>{item.acknowledgement_requested && <StatusBadge tone={acknowledged ? 'success' : 'warning'}>{acknowledged ? 'Acknowledged' : 'Acknowledgement requested'}</StatusBadge>}</div><p>{item.body}</p>{item.source_reference && <small>Source: {item.source_reference}</small>}</div>{organization?.type === 'supplier' && canAcknowledgeRequirement && item.acknowledgement_requested && <div className='partRequirementList__actions'><Button variant='secondary' disabled={acknowledged || mutating} onClick={() => run(() => dispatch(acknowledgePartRequirement(idOf(revisionDetail.review), idOf(item), { partId, revisionId })), 'Requirement acknowledged.')}>{acknowledged ? 'Acknowledged' : 'Acknowledge requirement'}</Button></div>}</article>})}</div> : <div className='partWorkspaceEmpty'><Check aria-hidden='true' /><h3>No structured requirements</h3><p>The released files remain the controlled technical definition.</p></div>}
        {revisionDetail.review && <div className='partReviewPanel'><div><p className='technicalLabel'>{organization?.type === 'supplier' ? 'Complete technical package review' : 'Supplier technical package review'}</p><h3>{organization?.type === 'supplier' ? `Step ${revisionDetail.review.state === 'not_started' ? '1' : unacknowledged.length ? '2' : revisionDetail.review.state === 'acknowledged' ? 'Complete' : '3'} · ${formatLabel(revisionDetail.review.state)}` : formatLabel(revisionDetail.review.state)}</h3><p>{organization?.type === 'supplier' ? 'Start the review, acknowledge every specifically requested requirement, then acknowledge the complete released package. Production acceptance remains a separate commitment.' : `${partnerName} has acknowledged ${acknowledgedIds.size} of ${requestedRequirements.length} requested requirements. Revision acknowledgement confirms review of the complete released package.`}</p>{organization?.type === 'supplier' && revisionDetail.review.state !== 'acknowledged' && <ol className='partReviewSteps'><li className={revisionDetail.review.state !== 'not_started' ? 'is-complete' : 'is-current'}><span>1</span>Start technical review</li><li className={unacknowledged.length === 0 ? 'is-complete' : revisionDetail.review.state !== 'not_started' ? 'is-current' : ''}><span>2</span>Acknowledge requested requirements</li><li className={revisionDetail.review.state === 'acknowledged' ? 'is-complete' : revisionDetail.review.state !== 'not_started' && unacknowledged.length === 0 ? 'is-current' : ''}><span>3</span>Acknowledge released package</li></ol>}</div>{organization?.type === 'supplier' && canReviewPart && <div>{revisionDetail.review.state === 'not_started' && <Button onClick={() => run(() => dispatch(startPartReview(revisionDetail.review.id)), 'Technical review started.')}>Start technical review</Button>}{['not_started', 'in_review', 'changes_requested'].includes(revisionDetail.review.state) && <Button variant='secondary' onClick={() => setReviewChangesOpen(true)}>Request changes</Button>}{['in_review', 'changes_requested'].includes(revisionDetail.review.state) && <Button disabled={unacknowledged.length > 0 || mutating} onClick={() => run(() => dispatch(acknowledgePartRevision(revisionDetail.review.id)), 'Released package acknowledged.')}>Acknowledge released package</Button>}</div>}</div>}
      </section>}
    ...
    <PartCaseDrawer v2Enabled={v2Enabled} onOpenFormal={async formalId => { await closeCase(); router.replace({ pathname: router.pathname, query: { ...router.query, collaboration: undefined, formal: formalId } }, undefined, { shallow: true }) }} open={caseDrawer.open} mode={caseDrawer.mode} itemDetail={caseDetail} shares={currentShare ? [currentShare] : []} productionRecords={productionRecords} defaultProductionRecordIds={defaultProductionRecordIds} lockProductionContext selectedAnchor={pendingAnchor} sourceAsset={viewer.asset} linkedVisual={caseVisual} itarControlled={revision.export_control === 'itar'} pending={mutating} upload={upload} feedback={caseFeedback} organizationType={organization?.type} relatedCompanyName={relatedCompanyName} onClose={closeCase} onOpenAnchor={openCaseAnchor} onVisualPreviewReady={saveCaseVisualPreview} onCreate={createCase} onMessage={body => runCase(() => dispatch(postPartCollaborationMessage(caseDrawer.id, body)), 'Reply added.')} onUpdate={payload => runCase(() => dispatch(updatePartCollaboration(caseDrawer.id, payload)), 'Responsibility updated.')} onAction={(action, note, version) => runCase(() => dispatch(applyPartCollaborationAction(caseDrawer.id, { action, note, version: version ?? caseDetail.item.version })), 'Workflow decision recorded.')} onUpload={(file, itar) => runCase(() => dispatch(uploadPartCollaborationAttachment(caseDrawer.id, { file, itar: revision.export_control === 'itar' ? itar : {} })), 'Evidence attached.')} onDownloadAttachment={downloadCaseAttachment} onArchive={async item => { const answer = await ask({ title: 'Archive closed case?', description: 'The case will leave active views and its history will be retained.', confirmLabel: 'Archive case', danger: true, fields: [{ name: 'reason', label: 'Reason for archiving', multiline: true }] }); if (!answer) return null; const reason = answer.reason.trim(); if (!reason) return null; return runCase(() => dispatch(archivePartCollaboration(idOf(item), { reason, version: item.version })), 'Closed case archived.').then(result => { if (result?.ok) closeCase(); return result }) }} onPromote={payload => runCase(() => dispatch(promotePartCollaboration(caseDrawer.id, { ...payload, production_record_id: String(record.id) })), 'Conversation escalated. Continue in the linked formal record.')} />
