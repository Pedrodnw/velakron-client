...
const PartAssetViewer = ({ asset, source, loading, annotationMode = false, anchors = [], caseMarkers = [], selectedAnchorId = '', onSelect, onOpenCase, onPreviewReady, onThumbnailReady }) => {
  ...
  if (['step', 'stp', 'stl'].includes(extension)) return <ModelViewer file={file} source={source} annotationMode={annotationMode} anchors={anchors} caseMarkers={caseMarkers} selectedAnchorId={selectedAnchorId} selectedAnchor={selectedAnchor} onSelect={onSelect} onOpenCase={onOpenCase} onPreviewReady={onPreviewReady} onThumbnailReady={onThumbnailReady} />
