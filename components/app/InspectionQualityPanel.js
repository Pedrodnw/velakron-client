...
const ResultEntry = ({ characteristic, sampleNumber, result, pending, onSave, onConfirm, onEvidence, isNext, readOnly = false, canConfirmFailure = false }) => {
...
export const InspectionRunWorkspace = ({ runId, production, organizationType, onClose, onChanged, embedded = false }) => {
  ...
  const hasReviewPermission = useSelector(getHasPermission('inspection.package.review'))
  const canConfirmFailure = useSelector(getHasPermission('inspection.failure.confirm'))
  ...
  const ownsEditableRun = run.current_actor_side === organizationType && ['not_started', 'in_progress', 'ready_to_submit', 'changes_requested'].includes(run.state)
  const canRecord = hasRecordPermission && ownsEditableRun
  const canSubmit = hasSubmitPermission && ownsEditableRun
  const canReview = hasReviewPermission && organizationType === 'oem' && run.current_actor_side === 'oem' && run.state === 'submitted'
...
const InspectionQualityPanel = ({ production, organizationType, embedded = false }) => {
  const dispatch = useDispatch()
  const canRecord = useSelector(getHasPermission('inspection.result.record'))
  const canReview = useSelector(getHasPermission('inspection.package.review'))
  const canSubmit = useSelector(getHasPermission('inspection.package.submit'))
