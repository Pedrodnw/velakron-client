...
const InspectionPlanPanel = ({ partId, revisionId, revision, organizationType, selectedAnchor, onRequestVisualContext, onOpenAnchor }) => {
  ...
  const effectiveManage = canManage && !supplierPreview
  ...
  if (!plan) return <section className='partWorkspacePanel inspectionPlanPanel inspectionPlanEmpty'><ClipboardCheck aria-hidden='true' /><p className='technicalLabel'>Collaborative inspection</p><h2>{canManage || (organizationType === 'oem' && revision?.lifecycle_state === 'draft') ? 'Turn design intent into executable checkpoints' : 'No inspection plan on this revision'}</h2><p>{organizationType === 'oem' && revision?.lifecycle_state === 'draft' ? 'Define exactly what the supplier must inspect, how often, and what evidence is required. The plan freezes with the revision.' : 'This released revision does not require structured inspection checkpoints.'}</p>{organizationType === 'oem' && revision?.lifecycle_state === 'draft' && <Button onClick={() => act(() => dispatch(createInspectionPlan(partId, revisionId, { title: `${revision?.revision || ''} inspection plan`, default_stage: 'final', default_sample_policy: { strategy: 'first_piece' } })), 'Inspection plan created.')}><Plus aria-hidden='true' /> Create inspection plan</Button>}</section>
  return <section className='partWorkspacePanel inspectionPlanPanel'>
    <header><div><p className='technicalLabel'>{supplierPreview ? 'Supplier preview' : 'Released quality definition'}</p><h2>{plan.title}</h2><p>{supplierPreview ? 'This is the read-only plan and guidance the connected supplier will receive.' : plan.description || 'Inspection checkpoints, sampling, evidence, and workflow gates for this exact part revision.'}</p></div><div className='inspectionPlanPanel__actions'>{effectiveManage && <Button onClick={() => openNew(selectedAnchor)}><Plus aria-hidden='true' /> Add checkpoint</Button>}{organizationType === 'oem' && <Button variant='secondary' onClick={() => { setSupplierPreview(value => !value); setSelected([]) }}><Eye aria-hidden='true' /> {supplierPreview ? 'Return to authoring' : 'Preview as supplier'}</Button>}<Button variant='secondary' onClick={() => act(() => dispatch(validateInspectionPlan(idOf(plan))), 'Plan validation completed.').then(result => { const validation = result?.payload?.data; if (validation) setFeedback({ type: validation.valid ? 'success' : 'error', message: validation.valid ? `Plan ready · ${validation.plan_hash.slice(0, 12)}…` : validation.errors.map(item => item.message).join(' ') }) })}><ShieldCheck aria-hidden='true' /> Validate</Button></div></header>
