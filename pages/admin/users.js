...
const Users = () => {
  ...
  return <>
    ...
    <FilterBar onSubmit={submit} actions={<Button type='submit' disabled={users.loading}><Search aria-hidden='true' /> Search</Button>}>
      ...
      <label><span>Role</span><select value={filters.role} onChange={event => setFilters(value => ({ ...value, role: event.target.value }))}><option value=''>All roles</option><option value='founder'>Founder</option><option value='oem_admin'>OEM administrator</option><option value='oem_user'>OEM member</option><option value='supplier_admin'>Supplier administrator</option><option value='supplier_user'>Supplier member</option><option value='velakron_admin'>Velakron administrator</option></select></label>
    </FilterBar>
