...
const Organizations = () => {
  ...
  return <>
    ...
    <FilterBar onSubmit={submitSearch} actions={<Button type='submit' disabled={loading}><Search aria-hidden='true' /> Search</Button>}>
      ...
      <label><span>Type</span><select value={filters.type} onChange={event => setFilters(value => ({ ...value, type: event.target.value }))}><option value=''>All types</option><option value='oem'>OEM</option><option value='supplier'>Supplier</option><option value='sales_partner'>Sales Partner</option><option value='velakron'>Velakron</option></select></label>
