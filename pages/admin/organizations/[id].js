...
const OrganizationDetail = () => {
  ...
  return <>
    ...
    {loading && !organization ? <section className='appPanel'><AppSkeleton lines={10} /></section> : organization ? <>
      ...
      <div className='appDashboardGrid'>
        <section className='appPanel'><header className='appPanel__header'><div><p className='technicalLabel'>First administrator</p><h2>Issue invitation</h2></div></header><form className='drawerForm' onSubmit={submitInvite}><div className='authForm__row'><FormField id='organization-admin-first-name' label='First name' value={invite.first_name} onChange={event => setInvite(value => ({ ...value, first_name: event.target.value }))} maxLength={80} autoComplete='given-name' /><FormField id='organization-admin-last-name' label='Last name' value={invite.last_name} onChange={event => setInvite(value => ({ ...value, last_name: event.target.value }))} maxLength={80} autoComplete='family-name' /></div><FormField id='organization-admin-email' label='Email address' type='email' value={invite.email} onChange={event => setInvite(value => ({ ...value, email: event.target.value }))} maxLength={320} autoComplete='email' required /><Button type='submit' disabled={mutating}>{mutating ? <LoaderCircle className='spin' aria-hidden='true' /> : <MailPlus aria-hidden='true' />} Issue administrator invitation</Button></form></section>
        <section className='appPanel'><header className='appPanel__header'><div><p className='technicalLabel'>Pending access</p><h2>Invitations</h2></div></header>{detail.pending_invitations?.length ? <div className='invitationList'>{detail.pending_invitations.map(item => <article key={item.id || item._id || item.email}><div><strong>{item.email}</strong><span>{formatRole(item.role)} · expires {formatDate(item.expires_at)}</span></div><Button variant='secondary' onClick={() => revokeInvitation(item)}>Revoke</Button></article>)}</div> : <EmptyState compact title='No pending invitations' description='Issue the first administrator invitation when the company is ready.' />}</section>
      </div>
      ...
      <section className='appPanel'><header className='appPanel__header'><div><p className='technicalLabel'>Security history</p><h2>Recent audited activity</h2></div></header>{detail.recent_audit_events?.length ? <div className='auditEventList'>{detail.recent_audit_events.map(event => <AuditEventRow key={event.id || event._id} event={event} />)}</div> : <EmptyState compact title='No audit activity' description='Security-relevant actions will appear here.' />}</section>
