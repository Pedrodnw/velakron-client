...
const PartWorkspace = () => {
  ...
  const openCase = caseItem => {
    ...
    setCaseDrawer({ open: true, mode: 'detail', id: caseItem.id || caseItem._id })
  }
  ...
  return <>
    ...
    <ResponsiveDrawer open={drawer === 'asset'} title='Upload technical file' onClose={() => setDrawer('')}><form className='partDrawerForm' onSubmit={event => { event.preventDefault(); if (!assetUpload.file || !assetUpload.role) return; run(() => dispatch(uploadPartAsset(id, revisionId, { file: assetUpload.file, role: assetUpload.role, label: assetUpload.label, isPrimary: assetUpload.is_primary, itar: revision.export_control === 'itar' ? { itar_upload_authorized: assetUpload.authorized, synthetic_data_acknowledged: assetUpload.authorized } : {} })), `Technical file uploaded as ${formatLabel(assetUpload.role)} and verified.`).then(result => { if (result?.ok) setAssetUpload(emptyAssetUpload) }) }}><div className='partUploadSequence'><span className={assetUpload.file ? 'is-complete' : 'is-current'}>1 <small>Choose file</small></span><span className={assetUpload.file && assetUpload.role ? 'is-complete' : assetUpload.file ? 'is-current' : ''}>2 <small>Confirm role</small></span><span>3 <small>Upload</small></span></div><label className='fileDropField'><FileUp aria-hidden='true' /><span><strong>{assetUpload.file?.name || 'Choose technical file'}</strong><small>STEP, STP, STL, PDF, PNG, JPEG, WebP, or text</small></span><input type='file' required onChange={event => handleAssetFile(event.target.files?.[0] || null)} /></label>{assetUpload.suggestion?.message && <div className={`partUploadSuggestion partUploadSuggestion--${assetUpload.suggestion.confidence}`}><Info aria-hidden='true' /><span><strong>{assetUpload.suggestion.confidence === 'detected' ? 'File type detected' : 'File role required'}</strong><small>{assetUpload.suggestion.message}</small></span></div>}<label className='selectField'><span>File role</span><select required value={assetUpload.role} onChange={event => setAssetUpload(current => ({ ...current, role: event.target.value, is_primary: event.target.value === 'primary_model' ? true : ['drawing'].includes(event.target.value) ? current.is_primary : false }))}><option value=''>Choose file role</option>{['primary_model', 'alternate_model', 'drawing', 'specification', 'inspection_plan', 'reference', 'other'].map(value => <option key={value} value={value}>{formatLabel(value)}</option>)}</select></label><FormField id='asset-label' label='File label (optional)' value={assetUpload.label} onChange={event => setAssetUpload(current => ({ ...current, label: event.target.value }))} />{['primary_model', 'drawing'].includes(assetUpload.role) && <label className='productionCheck'><input type='checkbox' checked={assetUpload.is_primary} onChange={event => setAssetUpload(current => ({ ...current, is_primary: event.target.checked }))} /><span><strong>Primary {assetUpload.role === 'drawing' ? 'drawing' : 'model'}</strong><small>Use this file by default when revision {revision?.revision} opens.</small></span></label>}{revision?.export_control === 'itar' && <label className='productionCheck'><input type='checkbox' checked={assetUpload.authorized} onChange={event => setAssetUpload(current => ({ ...current, authorized: event.target.checked }))} /><ShieldAlert aria-hidden='true' /><span><strong>I am authorized to upload this ITAR-controlled data</strong><small>Local preview environments accept synthetic data only.</small></span></label>}{upload && <p className='uploadProgress'><LoaderCircle className='spin' aria-hidden='true' /> {upload.filename} · {upload.progress}%</p>}<footer><Button variant='secondary' onClick={() => setDrawer('')}>Cancel</Button><Button type='submit' disabled={mutating || !assetUpload.file || !assetUpload.role || (revision?.export_control === 'itar' && !assetUpload.authorized)}>Upload as {assetUpload.role ? formatLabel(assetUpload.role) : 'selected role'}</Button></footer></form></ResponsiveDrawer>
