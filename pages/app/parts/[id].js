  ...
  return <>
    ...
    <section className='partWorkspaceShell'>
      ...
      {['model', 'drawing'].includes(tab) && <div className='partVisualWorkspace'>
        ...
        <main className='partViewerStage'><div className='partViewerStage__toolbar'><div><p className='technicalLabel'>Contextual review</p><strong>{viewer.asset ? attachmentName(viewer.asset) : `Open a ${tab}`}</strong></div>{viewer.asset && <div><Button variant={annotationMode ? 'primary' : 'secondary'} onClick={() => setAnnotationMode(value => !value)}><CircleDot aria-hidden='true' /> {annotationMode ? 'Cancel selection' : 'Select feature'}</Button><Button variant='secondary' onClick={() => downloadAsset(viewer.asset)}><Download aria-hidden='true' /> Download</Button></div>}</div><PartAssetViewer asset={viewer.asset} source={viewer.source} loading={viewer.loading} annotationMode={annotationMode} anchors={anchors.filter(anchor => !anchor.source_asset || collaborationIdOf(anchor.source_asset) === collaborationIdOf(viewer.asset))} caseMarkers={viewerCaseMarkers} selectedAnchorId={selectedAnchorId} onSelect={chooseVisualAnchor} onOpenCase={openCase} /></main>
