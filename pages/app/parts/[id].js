...
const PartWorkspace = () => {
  ...
  if (revision?.lifecycle_state === 'draft') nextStep = {
    ...
    action: assets.length ? (allowedActions.can_release_revision ? { label: 'Review release', kind: 'release' } : null) : (allowedActions.can_edit_revision ? { label: 'Upload first file', kind: 'tab', tab: 'files' } : null),
  }
    ...
    action: canCreateProduction ? { label: 'Use in production', kind: 'production' } : null,
  }
  ...
  return <>
    ...
    <section className='partWorkspaceShell'>
      ...
      {tab === 'overview' && <section className='partOverview'>
        ...
        <article className='partWorkspacePanel'><header><div><p className='technicalLabel'>Revision readiness</p><h2>{revision?.lifecycle_state === 'draft' ? 'Definition in progress' : revision?.lifecycle_state === 'withdrawn' ? 'Unavailable for new work' : 'Approved technical baseline'}</h2><p>{revision?.lifecycle_state === 'draft' ? 'Complete the technical package and release it before using it in production.' : revision?.lifecycle_state === 'withdrawn' ? 'Existing production records keep their historical copy of this revision.' : 'Supplier access and collaboration begin automatically when this revision is assigned in a production record.'}</p></div></header><div className='partOverviewActions'>{revision?.lifecycle_state === 'draft' ? allowedActions.can_release_revision && <Button onClick={openReleaseReview}>Review release</Button> : revision?.lifecycle_state === 'released' ? canCreateProduction && <Button href={`/app/production/new?part_revision_id=${revisionId}`}><Send aria-hidden='true' /> Use in production</Button> : revisionIdOf(part.current_released_revision) && revisionIdOf(part.current_released_revision) !== revisionId ? <Button variant='secondary' onClick={() => setRevisionId(revisionIdOf(part.current_released_revision))}>Open current release</Button> : null}</div></article>
        <article className='partWorkspacePanel partOverview__wide'><header><div><p className='technicalLabel'>Production reuse</p><h2>Related production records</h2><p>Each linked commitment keeps a frozen reference to this revision and manifest.</p></div>{canCreateProduction && organization.type === 'oem' && revision?.lifecycle_state === 'released' && <Button href={`/app/production/new?part_revision_id=${revisionId}`}><Plus aria-hidden='true' /> Create production record</Button>}</header>{productionRecords.length ? <div className='partOverviewRecords'>{productionRecords.map(record => <Button key={record.id || record._id} href={`/app/production/${record.id || record._id}`} variant='secondary'><span><strong>{record.public_reference || record.po_number}</strong><small>{formatLabel(record.current_stage)} · {formatLabel(record.lifecycle_state)}</small></span><ChevronRight aria-hidden='true' /></Button>)}</div> : <div className='partWorkspaceEmpty'><Box aria-hidden='true' /><h3>No production records use this revision yet</h3><p>{organization.type === 'oem' ? 'Released definitions can be selected when an OEM creates a production commitment.' : 'Production records linked to this shared revision will appear here.'}</p></div>}</article>
      </section>}
