...
import { AppPageHeader, AppSkeleton, ConfidentialityPanel, ErrorState, PermissionDenied, ProductionAttentionPanel, ProductionCollaborationPanel, ProductionStageStepper, ResponsiveDrawer, ScheduleHealthBadge, StatusBadge } from '../../../components/app'
...
const DeliveryForm = ({ record, pending, feedback, onSubmit }) => {
  ...
  return <form className='drawerForm' onSubmit={event => { event.preventDefault(); onSubmit({ delivered_date: date, note, version: record.version, idempotency_key: requestKey('delivery') }) }}>
    <p>Confirm this only after the OEM has received the shipment. This completes and locks the record.</p>
    <FormMessage type={feedback?.type}>{feedback?.message}</FormMessage>
...
const ForecastForm = ({ record, pending, feedback, onSubmit }) => {
  ...
  return <form className='drawerForm' onSubmit={event => { event.preventDefault(); onSubmit({ ...form, version: record.version, idempotency_key: requestKey('forecast') }) }}>
    ...
    <label className='productionCheck'><input type='checkbox' checked={form.report_issue} onChange={event => set('report_issue', event.target.checked)} /><span><strong>This update includes an issue that needs attention</strong><small>The OEM will see the issue separately from the schedule calculation.</small></span></label>
    {form.report_issue && <><label className='textAreaField' htmlFor='forecast-issue'><span>Issue explanation</span><textarea id='forecast-issue' value={form.issue} onChange={event => set('issue', event.target.value)} minLength={8} maxLength={1000} required /></label><label className='selectField' htmlFor='forecast-severity'><span>Priority</span><select id='forecast-severity' value={form.severity} onChange={event => set('severity', event.target.value)}><option value='high'>High</option><option value='medium'>Medium</option><option value='low'>Low</option></select></label></>}
...
const AttentionForm = ({ pending, feedback, organizationType, onSubmit }) => {
  ...
  return <form className='drawerForm' onSubmit={event => { event.preventDefault(); onSubmit({ explanation, severity }) }}>
    <p>{organizationType === 'supplier' ? 'Report a shared production issue so the OEM can act on it.' : 'Flag an OEM-internal follow-up without exposing the note to the supplier.'}</p>
    <FormMessage type={feedback?.type}>{feedback?.message}</FormMessage>
    ...
    <label className='selectField' htmlFor='attention-severity'><span>Priority</span><select id='attention-severity' value={severity} onChange={event => setSeverity(event.target.value)}><option value='high'>High</option><option value='medium'>Medium</option><option value='low'>Low</option></select></label>
    <FormActions pending={pending} submitLabel='Add attention flag' icon={AlertTriangle} />
  </form>
}
...
const ProductionRecordDetail = () => {
  ...
  return <>
    ...
    <ConfidentialityPanel
      ...
      onUploadNda={payload => runInline(() => dispatch(uploadConfidentialityNda('production', record.id, payload)), 'Production NDA uploaded. A new supplier signature is required.')}
    />
    ...
    <div className='productionStatusStrip'>
      ...
      <div><span>Stage</span><StatusBadge tone={statusTone(record.current_stage)}>{formatLabel(record.current_stage || 'Not assigned')}</StatusBadge></div>
    ...
    <ProductionAttentionPanel conditions={collaboration?.attention || []} canAcknowledge={canAcknowledgeAttention} canResolve={canResolveAttention} pending={collaboration?.mutating} onAcknowledge={item => runInline(() => dispatch(acknowledgeProductionAttention(record.id, item.id)), 'Attention reason acknowledged.')} onResolve={item => { setActionTarget(item); setDrawer('resolve-attention') }} />
    ...
    <ResponsiveDrawer open={Boolean(drawer)} title={{ accept: 'Review assignment', decline: 'Decline assignment', assign: record.supplier_organization ? 'Reassign supplier' : 'Assign supplier', machine: 'Primary machine', stage: 'Update production stage', forecast: 'Update shipping forecast', attention: 'Flag for attention', 'resolve-attention': 'Resolve attention reason', 'archive-note': 'Archive note', 'archive-attachment': 'Archive file', edit: 'Edit production record', delivery: 'Confirm delivery', cancel: 'Cancel production record', reopen: 'Reopen production record', archive: 'Archive production record' }[drawer] || 'Production action'} onClose={closeDrawer}>
      ...
      {drawer === 'forecast' && <ForecastForm record={record} pending={collaboration?.mutating} feedback={feedback} onSubmit={async payload => {
        const { report_issue: reportIssue, issue, severity, ...forecast } = payload
        const updated = await runInline(() => dispatch(updateProductionForecast(record.id, forecast)), 'Shipping forecast updated.')
        if (!updated) return false
        if (reportIssue) await runInline(() => dispatch(reportProductionAttention(record.id, { explanation: issue, severity })), 'Shipping forecast and issue reported.')
        setDrawer(null)
        return true
      }} />}
      ...
      {drawer === 'archive-attachment' && <ReasonForm pending={collaboration?.mutating} feedback={feedback} description='The file will no longer be downloadable, while its audit history remains.' submitLabel='Archive file' onSubmit={reason => run(() => dispatch(archiveProductionAttachment(record.id, actionTarget.id, reason)), 'File archived.')} />}
      ...
      {drawer === 'delivery' && <DeliveryForm record={record} pending={pending} feedback={feedback} onSubmit={payload => run(() => dispatch(confirmProductionDelivery(record.id, payload)), 'Delivery confirmed.')} />}
