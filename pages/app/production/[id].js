...
import FormalEscalationPanel from '../../../components/app/FormalEscalationPanel'
import { isFormalV2 } from '../../../store/collaborationV2'
...
const RevisionChangeActionForm = ({ change, organizationType, pending, blocked, feedback, onSubmit }) => {
  ...
  return <form className='drawerForm'>
    ...
    {blocked && <p role='status'>The OEM must release every active Production Block before applying a revision.</p>}
    <div className='drawerForm__actions'>{actions.map(([key, label]) => <Button key={key} variant={['reject', 'cancel'].includes(key) ? 'secondary' : 'primary'} disabled={pending || (blocked && key === 'apply') || (key !== 'apply' && key !== 'approve' && note.trim().length < 3)} onClick={() => onSubmit(key, note)}>{label}</Button>)}</div>
  </form>
}
...
const ForecastForm = ({ record, pending, feedback, onSubmit, v2Enabled }) => {
  const [form, setForm] = useState({
  ...
  return <form className='drawerForm' onSubmit={event => { event.preventDefault(); onSubmit({ ...form, version: record.version, idempotency_key: requestKey('forecast') }) }}>
    ...
    {!v2Enabled && <label className='productionCheck'><input type='checkbox' checked={form.report_issue} onChange={event => set('report_issue', event.target.checked)} /><span><strong>This update includes a flag for the OEM</strong><small>The selected category will control its schedule risk automatically.</small></span></label>}
...
const ProductionRecordDetail = () => {
  ...
  const activeProductionBlock = (collaboration?.attention || []).some(item => item.category === 'production_block' && item.active && (!isFormalV2(item) || item.blocking === true))
  const openFormalCreation = () => router.replace({ pathname: router.pathname, query: { ...router.query, formal: 'new' } }, undefined, { shallow: true })
  const actionButtons = <>
    {detail?.actions?.accept && <Button onClick={() => setDrawer('accept')}><Check aria-hidden='true' /> Review assignment</Button>}
    {detail?.actions?.transition && <Button disabled={activeProductionBlock} aria-describedby={activeProductionBlock ? 'production-block-notice' : undefined} onClick={() => setDrawer('stage')}><RefreshCw aria-hidden='true' /> Update stage</Button>}
    ...
    {detail?.actions?.confirm_delivery && <Button disabled={activeProductionBlock} aria-describedby={activeProductionBlock ? 'production-block-notice' : undefined} onClick={() => setDrawer('delivery')}><PackageCheck aria-hidden='true' /> Confirm delivery</Button>}
    {detail?.actions?.approve_quality && <Button disabled={activeProductionBlock} aria-describedby={activeProductionBlock ? 'production-block-notice' : undefined} onClick={() => setDrawer('quality-approval')}><Check aria-hidden='true' /> Approve parts</Button>}
    {detail?.actions?.report_quality_issue && <Button variant='secondary' onClick={v2Enabled ? openFormalCreation : () => setDrawer('quality-issue')}><AlertTriangle aria-hidden='true' /> Report quality issue</Button>}
    {detail?.actions?.edit && <Button variant='secondary' disabled={activeProductionBlock} aria-describedby={activeProductionBlock ? 'production-block-notice' : undefined} onClick={() => setDrawer('edit')}><Edit3 aria-hidden='true' /> Edit</Button>}
    {detail?.actions?.assign && organization.type === 'oem' && <Button variant='secondary' disabled={activeProductionBlock} aria-describedby={activeProductionBlock ? 'production-block-notice' : undefined} onClick={() => setDrawer('assign')}><UserRoundCheck aria-hidden='true' /> {record.supplier_organization ? 'Reassign' : 'Assign'}</Button>}
    {detail?.actions?.assign_machine && <Button variant='secondary' onClick={() => setDrawer('machine')}><Cog aria-hidden='true' /> Machine</Button>}
    {canReportAttention && <Button variant='secondary' onClick={v2Enabled ? openFormalCreation : () => setDrawer('attention')}><AlertTriangle aria-hidden='true' /> {v2Enabled ? 'Create formal record' : 'Flag attention'}</Button>}
  </>
  ...
  return <>
    ...
    {activeProductionBlock && <div id='production-block-notice' role='status' className='supplierStateNotice supplierStateNotice--changes_requested'><AlertTriangle aria-hidden='true' /><div><strong>Production is blocked</strong><p>Production progress, quantity changes, inspections, revision changes, and delivery are locked. The OEM must release every active Block. Solution approval alone does not release production. Messages, evidence, and shipping forecasts remain available.</p></div></div>}
    ...
    <ProductionAttentionPanel conditions={(collaboration?.attention || []).filter(item => !isFormalV2(item))} canAcknowledge={canAcknowledgeAttention} canResolve={item => canResolveAttention && (item.workflow?.managed || organization.type !== 'supplier' || item.source === 'supplier')} pending={collaboration?.mutating} onAcknowledge={item => runInline(() => dispatch(acknowledgeProductionAttention(record.id, item.id)), 'Attention reason acknowledged.')} onResolve={item => { setActionTarget(item); setDrawer('resolve-attention') }} onWorkflowAction={(item, action) => { setActionTarget({ item, action }); setDrawer('attention-workflow-action') }} />
    {['oem', 'supplier'].includes(organization.type) && record.supplier_organization && <FormalEscalationPanel record={record} organization={organization} enabled={v2Enabled} files={collaboration?.attachments || []} onDownload={(file, attestation) => dispatch(requestAttachmentDownload(record.id, file.id, attestation))} />}
    <div className='productionDetailGrid'>
      <ProductionPartWorkspace record={record} organization={organization} onEditDetails={detail?.actions?.edit && !activeProductionBlock ? () => setDrawer('edit') : null} onModelThumbnailReady={setModelThumbnailPreview} />
      ...
      {organization.type === 'oem' && (detail?.actions?.cancel || detail?.actions?.reopen || detail?.actions?.archive) && <section className='appPanel productionLifecycleActions'><header className='appPanel__header'><div><p className='technicalLabel'>Record controls</p><h2>Lifecycle</h2></div></header><div>{detail.actions.cancel && <Button variant='secondary' onClick={() => setDrawer('cancel')}><X aria-hidden='true' /> Cancel record</Button>}{detail.actions.reopen && <Button variant='secondary' disabled={activeProductionBlock} aria-describedby={activeProductionBlock ? 'production-block-notice' : undefined} onClick={() => setDrawer('reopen')}><RotateCcw aria-hidden='true' /> Reopen record</Button>}{detail.actions.archive && <Button variant='secondary' disabled={activeProductionBlock} aria-describedby={activeProductionBlock ? 'production-block-notice' : undefined} onClick={() => setDrawer('archive')}><Archive aria-hidden='true' /> Archive record</Button>}</div></section>}
    </div>
