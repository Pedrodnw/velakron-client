...
import { inspectionSelectors, loadInspectionPlan } from '../../../store/slices/entities/inspection'
...
const NewProductionRecord = () => {
  ...
  const [selectedRevisionId, setSelectedRevisionId] = useState(String(router.query.part_revision_id || ''))
  const inspectionPlanSummary = useSelector(inspectionSelectors.getPlan(selectedRevisionId))
  ...
  return <>
    ...
    <ProductionRecordForm relationships={relationships} partWorkspaces={partWorkspaces} initialPartRevisionId={String(router.query.part_revision_id || '')} inspectionPlanSummary={inspectionPlanSummary} pending={pending} feedback={feedback} workflow={workflow} itarCapability={workflow?.compliance?.itar} onPartRevisionSelected={(selected, revisionId) => { setSelectedRevisionId(String(revisionId || '')); if (selected && revisionId) dispatch(loadInspectionPlan(selected.id || selected._id, revisionId)) }} onSubmit={submit} />
  </>
}
