...
const Production = () => {
  ...
  const columns = [
    { key: 'part', label: 'Part', render: item => <div className='tablePrimary'><strong>{recordTitle(item)}</strong><span>{item.part_name || item.public_reference}</span>{item.export_control === 'itar' && <span className='itarTableMarker'>ITAR controlled</span>}</div> },
    ...
    { key: 'health', label: 'Schedule / next action', render: item => <><ScheduleHealthBadge value={item.schedule_health} /><ProductionPendingActions record={item} /></> },
  ...
  return <>
    ...
    <section className='productionMobileCards' aria-label='Production records'>{records.map(item => <RecordCard key={item.id} href={recordHref(item)} eyebrow={item.public_reference} title={<>{recordTitle(item)}{item.part_number && item.part_name && <><br />{item.part_name}</>}</>} description={company(item)} badges={<>{item.export_control === 'itar' && <StatusBadge tone='danger'>ITAR</StatusBadge>}<StageBadge value={item.current_stage || item.acceptance_status} /><ScheduleHealthBadge value={item.schedule_health} /><ProductionPendingActions record={item} /></>} facts={[{ label: 'PO', value: item.po_number }, { label: 'Required arrival', value: formatDate(item.required_delivery_date) }, { label: 'Expected ship', value: formatDate(item.expected_ship_date) }]} actionLabel='Open' />)}</section>
