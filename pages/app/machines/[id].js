import { ArrowLeft, Camera, Cog, Download, LoaderCircle, MapPin, Ruler } from 'lucide-react'
import { useRouter } from 'next/router'
import { useEffect, useState } from 'react'
import { useDispatch, useSelector } from 'react-redux'
import { AppPageHeader, AppSkeleton, EmptyState, ErrorState, PermissionDenied, StatusBadge } from '../../../components/app'
...
import { downloadMachinePhoto, loadMachine, machineSelectors, uploadMachinePhoto } from '../../../store/slices/entities/machines'
  ...
  return <>
    ...
    <div className='machineDetailGrid'>
      <section className='appPanel machinePhotoPanel'>
        ...
        {machine.photo_attachments?.length ? <div className='attachmentPlaceholders'>{machine.photo_attachments.map(photo => <div key={photo.id}><Camera aria-hidden='true' /><strong>{photo.display_filename || photo.original_filename}</strong><span>{photo.state === 'available' ? (photo.scan_status === 'unavailable' ? 'Format verified · prototype malware scan not enabled' : 'Security check complete') : `Security status: ${formatLabel(photo.state)}`}</span>{photo.state === 'available' && <button type='button' className='tableAction' onClick={() => downloadPhoto(photo)}><Download aria-hidden='true' /> Download</button>}</div>)}</div> : <EmptyState compact icon={Camera} title='No photos yet' description='Photos are optional. Prototype uploads are format-verified before download; malware scanning is not enabled, so do not upload regulated data.' />}
      </section>
