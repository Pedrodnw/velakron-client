...
function Insights({guides}) { return <>
  ...
  <PageHero eyebrow='Insights · practical manufacturing coordination' title='A clearer way to think about shared work.' action={false}><p>Guides for the production questions that cross company boundaries. Useful ideas to bring into the next supplier conversation.</p></PageHero>
  <section className='mk-section'><div className='mk-container mk-insights-list'>{guides.map((article,index)=><article key={article.slug} className={index===0?'mk-insight-featured':'mk-insight-row'}><div><p className='mk-eyebrow'>{article.topic}</p><h2><Link href={`/insights/${article.slug}`}>{article.title}</Link></h2><p>{article.summary}</p><span className='mk-article-meta'>Velakron · September 10, 2026 · {article.readMinutes} min read</span><Link className='mk-text-link' href={`/insights/${article.slug}`}>Read the guide<ArrowUpRight size={18} aria-hidden /></Link></div>{index===0 && <Link href={`/insights/${article.slug}`} tabIndex={-1} aria-hidden='true'><Image src={productMedia[article.media].src} width={productMedia[article.media].width} height={productMedia[article.media].height} alt='' sizes='(max-width: 767px) 90vw, 550px' /></Link>}</article>)}</div></section>
