...
const ImtsDemo = () => {
  ...
  const submit = async event => {
    ...
    setPending(true)
    setError('')
    const result = await dispatch(startTradeShowDemo(form))
    if (!result?.ok) {
      setPending(false)
  ...
  return <>
    <Seo
      ...
      description='Start a private OEM or Supplier Velakron IMTS experience.'
      path='/imts-demo'
      noIndex
    />
    <div className='tradeShowPage'>
      ...
      <main className='tradeShowExperience'>
        <section className='tradeShowExperience__story'>
          <p className='technicalLabel'>Live product experience</p>
